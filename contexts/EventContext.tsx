...
import { eventService, type EventSaveResult } from "@/services/eventService";
import {
...
export function EventProvider({ children }: PropsWithChildren) {
  ...
  const createEvent = useCallback(
    async (draft: EventDraft): Promise<EventSaveResult> => {
      const result = await eventService.createEvent({
        ...draft,
        source: "manual",
      });
      result.lineReminder = await lineIntegrationService.syncEventReminder(
        result.event,
      );
  ...
  const editEvent = useCallback(
    async (id: string, draft: EventDraft): Promise<EventSaveResult> => {
      ...
      setEvents((current) =>
        sortEvents(
          current.map((item) => (item.id === id ? result.event : item)),
        ),
      );
      return result;
    },
  ...
  const removeEvent = useCallback(
    async (id: string) => {
      ...
      if (existing)
        await lineIntegrationService
          .syncEventReminder({ ...existing, lineReminderEnabled: false })
          .catch(() => undefined);
  ...
  const simulateIncomingLineEvent = useCallback(async (): Promise<
    "ready" | "duplicate"
  > => {
  ...
  const acceptIncomingEvent = useCallback(
    async (draft?: EventDraft): Promise<EventSaveResult> => {
  ...
  const dismissIncomingEvent = useCallback(
    () => setIncomingEvent(undefined),
    [],
  );
